...
using AutofacServiceProvider = LTs.DependencyInjections.DependencyInjection.AutofacServiceProvider;
...
/// </summary>
public static class AutofacPopulateExtensions
{
    /// <summary>
    ///     Registers the services from the container in the builder.
    /// </summary>
    /// <param name="builder">Container builder to register the services into.</param>
    /// <param name="container">Container with the services to register.</param>
    /// <returns>Container builder with the services registered.</returns>
    [ UsedImplicitly ]
    public static ContainerBuilder Populate( this ContainerBuilder builder, IContainer container )
    {
    ...
    ///     If provided and not <see langword="null" /> then all registrations with lifetime
    ///     <see cref="ServiceLifetime.Singleton" /> are registered
    ///     using
    ///     <see cref="IRegistrationBuilder{TLimit,TActivatorData,TRegistrationStyle}.InstancePerMatchingLifetimeScope" />
    ...
    /// <summary>
    ///     Inspect each component registration, and determine whether we can avoid adding the
    ...
    private static void AddFromKeyedServiceParameterMiddleware( object? sender, ComponentRegisteredEventArgs e )
    {
        ...
        {
            if( !ex.Message.Contains( "has already been built" ) )
            {
                throw;
            }
        }
    ...
    ///     If not <see langword="null" /> then all registrations with lifetime <see cref="ServiceLifetime.Singleton" /> are
    ///     registered
    ///     using
    ///     <see cref="IRegistrationBuilder{TLimit,TActivatorData,TRegistrationStyle}.InstancePerMatchingLifetimeScope" />
